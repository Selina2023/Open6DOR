...
import random
import time
from utils import images_to_video, quat_axis, orientation_error, cube_grasping_yaw, read_yaml_config
import os
import yaml
...
# load franka asset
franka_asset_file = cfgs["franka_asset_file"]
asset_options = gymapi.AssetOptions()
...
asset = gym.load_asset(sim, asset_root, mpath, asset_options)
actor_num = num_envs
InitPose = [[0.0, 0.0, 0.1] for i in range(num_envs)]
InitQuat = [[random.uniform(-1, 1) for i in range(4)] for j in range(num_envs)]
...
for i in range(num_envs):
    ...
    initial_pose.p.x = table_pose.p.x + np.random.uniform(-0.2, 0.1)
    initial_pose.p.y = table_pose.p.y + np.random.uniform(-0.3, 0.3)
    initial_pose.p.z = table_dims.z + 1 * 0.05
    initial_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.random.uniform(-math.pi, math.pi))
    # initial_pose.p = gymapi.Vec3(InitPose[i][0], InitPose[i][1], InitPose[i][2]+table_dims.z + 0.5 * box_size)
    # initial_pose.r = gymapi.Quat(InitQuat[i][0], InitQuat[i][1], InitQuat[i][2], InitQuat[i][3])
    actor_handle = gym.create_actor(env, asset, initial_pose, 'actor', i, 1)
    actor_idx = gym.get_actor_rigid_body_index(env, actor_handle, 0, gymapi.DOMAIN_SIM)
    actor_idxs.append(actor_idx)
    gym.set_actor_scale(env, actor_handle, 0.5)
...
for frame in range(STEPS):
    ...
    box_pos = rb_states[actor_idxs, :3]
    box_rot = rb_states[actor_idxs, 3:7]
    ...
    gripped = (gripper_sep < 0.045) & (box_dist < grasp_offset + 0.5 * 0.05)
    ...
    if SAVE_VIDEO and frame <= STEPS:
        if frame >=1 :
            os.makedirs("../drafts/video/imgs", exist_ok=True)
            gym.write_viewer_image_to_file(viewer, "../drafts/video/imgs/frame_{:04d}.png".format(frame))
        if frame >= STEPS-1:
            os.makedirs("../drafts/video", exist_ok=True)
            images_to_video("../drafts/video/imgs", "../drafts/video/video.mp4")
            os.system("rm -r ../drafts/video/imgs/*.png")
