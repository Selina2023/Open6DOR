...
# except:
#     print("Skip importing GroundedSAM")
import cv2
import numpy as np
...
if __name__ == "__main__":
    ...
    config = "GroundedSAM/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py"
    import imageio
    img = imageio.imread(image_path)
    # img = imageio.
    model, predictor = prepare_GroundedSAM_for_inference(sam_version, sam_checkpoint, grounded_checkpoint, config, device)
    masks = inference_one_image(img, model, predictor, box_threshold=box_threshold, text_threshold=text_threshold, text_prompt=text_prompt, device=device)
    masked_img = img.copy()
    masked_img[masks[0][0].cpu().numpy()] = 255
    img = imageio.imwrite("test2.png",masked_img)
    import pdb; pdb.set_trace()
